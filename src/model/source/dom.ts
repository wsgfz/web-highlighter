 ...
 */
export const getTextChildByOffset = ($parent: Node, offset: number, robustMode: boolean = false): DomNode => {
...
const robustSearch = (
    ...
    try {
        const originalNode = startMeta.parentIndex === ROOT_IDX 
            ? $root 
    ...
    if (robustOptions?.enabled) {
        ...
        for (let i = 0; i < siblingNodes.length; i++) {
            const result = tryMatchInNode(siblingNodes[i], startMeta.textOffset);
            if (result) {
                return result;
            }
        }
...
export const queryElementNode = (
    ...
    if (searchResult) {
        return {
            start: $root, // Return root for compatibility, actual DOM info is in startInfo/endInfo
            end: $root,
    ...
    const start = startIndex === ROOT_IDX 
        ? $root 
        : $root.getElementsByTagName(hs.startMeta.parentTagName)[startIndex];
    const end = endIndex === ROOT_IDX 
        ? $root 
        : $root.getElementsByTagName(hs.endMeta.parentTagName)[endIndex];
